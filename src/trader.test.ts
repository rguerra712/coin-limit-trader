...
import {CoinType, Order, OrderDetails, OrderPlacedResult} from '../src/types/types';
...
describe('trade() with no cancellations', () => {
  ...
  it('Should call placed order on client with no cancellation call',
     async () => {
       ...
       mockOrderClient.mockImplementation(
           () => ({getOrderDetails: jest.fn(), cancelOrder: jest.fn()}));
       orderClientMock = new mockOrderClient();
       mockOrderPlacer.mockImplementation(() => ({placeOrder: jest.fn()}));
       orderPlacerMock = new mockOrderPlacer();
       const trader = new Trader(
           orderClientMock, orderPlacerMock, orderSchedulerMock,
           priceFinderMock);
  ...
  it('Should match placed order id', async () => {
    ...
    orderPlacerMock = new mockOrderPlacer();
    const trader = new Trader(
        orderClientMock, orderPlacerMock, orderSchedulerMock, priceFinderMock);
...
describe('trade() with previous order', () => {
  ...
  beforeEach(() => {
    ...
    mockPriceFinder = jest.fn<GdaxPriceFinder>(
        () => ({
          getCurrentPrice: jest.fn<Promise<number>>().mockReturnValue(
              Promise.resolve(123.45))
        }));
    orderClientMock = new mockOrderClient();
    orderPlacerMock = new mockOrderPlacer();
    orderSchedulerMock = new mockOrderScheduler();
  ...
  it('Should do nothing if order is no longer active', async () => {
    ...
    mockOrderClient.mockImplementation(
        () => ({
          getOrderDetails:
              jest.fn().mockReturnValue(Promise.resolve(orderDetails)),
          cancelOrder: jest.fn()
        }));
    orderClientMock = new mockOrderClient();
    mockOrderPlacer.mockImplementation(() => ({placeOrder: jest.fn()}));
